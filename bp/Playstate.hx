...
class PlayState extends MusicBeatState
{
	...
	var alertattack:FlxSprite;
	var attackhere:FlxSprite;
	var charahere:Bool = false;
	...
	override public function create()
	{
		...
		switch(stageCheck)
		{
			...
			case 'school':
			{
					...
					if(FlxG.save.data.distractions){
					 ...
					 //Alert!
				    alertattack = new FlxSprite();
				    alertattack.frames = Paths.getSparrowAtlas('ch_alert');
				    alertattack.animation.addByPrefix('alert', 'alert', 24, false);	
				    alertattack.antialiasing = true;
				    alertattack.setGraphicSize(Std.int(alertattack.width * 1.5));
				    alertattack.cameras = [camHUD];
				    alertattack.x = FlxG.width - 500;
				    alertattack.y = 205;
				    alertattack.animation.play("alert");   

					//Chara Gonna Kill YOU
                    attackhere = new FlxSprite();
				    attackhere.frames = Paths.getSparrowAtlas('slashatk');
				    attackhere.animation.addByPrefix('slash', 'attacklook', 30, false);	
				    attackhere.setGraphicSize(Std.int(attackhere.width * 1.15));
				    attackhere.antialiasing = true;
					attackhere.setPosition(900,500);
					}
			}
			case 'schoolEvil':
			{
		...
		if (FlxG.save.data.songPosition) // I dont wanna talk about this code :(
			{
				...
				if (PlayStateChangeables.useDownscroll)
					songName.y -= 3;
				songName.setFormat(Paths.font("DeterminationSansWebRegular-369X.ttf"), 20, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		kadeEngineWatermark.setFormat(Paths.font("DeterminationSansWebRegular-369X.ttf"), 20, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("DeterminationSansWebRegular-369X.ttf"), 20, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		replayTxt.setFormat(Paths.font("DeterminationSansWebRegular-369X.ttf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		botPlayState.setFormat(Paths.font("DeterminationSansWebRegular-369X.ttf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
    ...
    function attack(state:Bool = false):Void
	{
		trace("CHARA KILL YA");
		if(state){
			//Chara just want to kill lol
			add(attackhere);
			attackhere.animation.play('slash');
			FlxG.sound.play(Paths.sound('attack'), 1);
			attackhere.animation.finishCallback = function(pog:String)
			{
					remove(attackhere);
			}
	...
	function Alert():Void
	{
		trace("HEY YOU CALCIUM");
		add(alertattack);
		alertattack.animation.play('alert');
		FlxG.sound.play(Paths.sound('alert'), 1);
		alertattack.animation.finishCallback = function(pog:String)
		{
		 remove(alertattack);
		}
	...
	public function NearlyEquals(value1:Float, value2:Float, unimportantDifference:Float = 10):Bool
		...
		private function keyShit():Void // I've invested in emma stocks
			{
		    if(SONG.song.toLowerCase() == "variance"){
	...
	*/
	...
	override function stepHit()
	{
		...
		#end
		if (curSong.toLowerCase() == 'variance'){
			if(charahere)
			{
				if (SONG.notes[Math.floor(curStep / 16)].mustHitSection && !dad.animation.curAnim.name.startsWith("sing") && curStep % 2 == 0)
				{
					dad.dance();
				}
			}
